...
We use factory pattern when the class instantiation is dynamic,
and depends on an input
"""
...
class Shape(ABC):
    ...
    def calculateArea():
        """
        Override this abstract method with the formula to calculate
...
class ShapeFactory:
    ...
    def buildShape(shape):
        ...
        if shape.lower() not in ["square,circle"]:
            raise Exception(
                "Only the area for Squares and Circles can be calculated currently"
            )
