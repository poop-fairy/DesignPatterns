"""
Challange:
I have a weather service that receives temprature in Celcius,
but the American user can only understand Farhenite. 
...
class GetOpenWeatherAPI:
    ...
    def getAPI(self):
        """
        The API will not be uploaded to GitHub as I've added
        the JSON file with the API to .gitignore
...
class ICelciusToFarheniteAdapter(ABC):
    ...
    def convertCelciusToFarhenite(self):
        """
        Should convert temprature in Celcius to Farhenite
...
print(
    "Hi American, the current Temprature is in {fcity} is {temp} F".format(
        temp=round(temp_in_farhenite.convertCelciusToFarhenite(), 2), fcity=city
    )
)
