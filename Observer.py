"""
Challange:
I have a stock ticker that should always be in sync with the stock prices
in the database.
...
class IObservable(ABC):
    ...
    def subscribe(self, Observer):
        """
        Add a list of Observers that will be notified when an Observable
        event gets updated here
        """
        pass
    ...
    def notifyObservers(self, Observer):
        pass  # Call Observer's update method
...
class StockMarket(IObservable):
    _observe = []
    dict_of_tracked_stocks = {"AAPL": 189.24, "MFST": 378.61, "META": 334.70}
...
class UserSideTicker(IObserver):
    ...
    def update(self, stock, price):
        print(
            ("Update for {dashboard}- ").format(dashboard=self.name), stock, ":", price
        )
